  ...
  generateSteps(graphDTO: GraphDTO, params: AlgorithmParams): Step[] {
    ...
    for (const node of graphDTO.nodes) {
      if (node.id === startNode) {
        ...
        this.addHighlightNodeStep(node.id, 'current', `Стартовая вершина: ${this.getNodeLabel(startNode)}`);
      } else {
    ...
    for (let iteration = 0; iteration < nodeCount - 1; iteration++) {
      ...
      for (const edge of edges) {
        ...
        this.addHighlightEdgeStep(edge.edgeId, 'active', `Проверка ребра ${this.getNodeLabel(u)} → ${this.getNodeLabel(v)} (вес: ${w})`);
        ...
        if (distU !== Infinity && distU + w < distV) {
          ...
          this.addHighlightNodeStep(v, 'current', `Обновление: расстояние до ${this.getNodeLabel(v)} = ${newDist}`);
          this.addUpdateNodeStep(
            v,
            { label: `${this.getNodeLabel(v)}: ${newDist}` },
            `Релаксация: ${this.getNodeLabel(u)} → ${this.getNodeLabel(v)}, новое расстояние = ${newDist}`
          );
          this.addHighlightEdgeStep(edge.edgeId, 'path', `Релаксация выполнена: ${this.getNodeLabel(u)} → ${this.getNodeLabel(v)}`);
        } else {
          this.addHighlightEdgeStep(edge.edgeId, 'visited', `Релаксация не требуется: ${this.getNodeLabel(u)} → ${this.getNodeLabel(v)}`);
        }
      ...
      if (!hasRelaxation) {
        this.addDescriptionStep(`Итерация ${iteration + 1}: изменений нет, алгоритм завершён досрочно`);
        break;
      }
    }
    ...
    for (const edge of edges) {
      ...
      this.addHighlightEdgeStep(edge.edgeId, 'active', `Проверка ребра ${this.getNodeLabel(u)} → ${this.getNodeLabel(v)} на отрицательный цикл`);

      if (distU !== Infinity && distU + w < distV) {
        hasNegativeCycle = true;
        negativeCycleEdges.push(edge.edgeId);
        this.addHighlightEdgeStep(edge.edgeId, 'rejected', `Обнаружено: ${this.getNodeLabel(u)} → ${this.getNodeLabel(v)} указывает на отрицательный цикл!`);
        this.addHighlightNodeStep(u, 'rejected', `Вершина ${this.getNodeLabel(u)} в отрицательном цикле`);
        this.addHighlightNodeStep(v, 'rejected', `Вершина ${this.getNodeLabel(v)} в отрицательном цикле`);
      } else {
        this.addHighlightEdgeStep(edge.edgeId, 'default');
      }
    }
    ...
    if (hasNegativeCycle) {
      ...
      for (const node of graphDTO.nodes) {
        if (node.id === startNode) {
          this.addHighlightNodeStep(node.id, 'path', `Стартовая вершина: ${this.getNodeLabel(startNode)}`);
        } else {
          const dist = this.distances.get(node.id);
          if (dist !== undefined && dist !== Infinity) {
            const path = this.reconstructPath(startNode, node.id);
            if (path.length > 1) {
              this.addHighlightNodeStep(node.id, 'path', `Кратчайшее расстояние: ${dist}, путь: ${path.map(n => this.getNodeLabel(n)).join(' → ')}`);
            }
          }
        }
      }
      ...
      for (const node of graphDTO.nodes) {
        ...
        if (pred !== null && pred !== undefined) {
          ...
          if (edge) {
            this.addHighlightEdgeStep(edge.edgeId, 'path', `Ребро в кратчайшем пути: ${this.getNodeLabel(pred)} → ${this.getNodeLabel(node.id)}`);
          }
        }
      }
    }
